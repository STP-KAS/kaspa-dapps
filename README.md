...
| [REPOS.md](REPOS.md) | All 42 STP-KAS repos: keep, one-of, archive |
...
Pick a layer. They are not interchangeable.
...
| Object | What it actually is | Status 15 Sep 2026 | Use it as |
| --- | --- | --- | --- |
...
Executable lesson (10/10 tests, this machine, 15 Sep 2026): [grok-heavy-showcase](https://github.com/STP-KAS/grok-heavy-showcase). Native redeem still works after the issuer freeze. Guest transfer fails. `useAsGas()` throws `NEVER_GAS`. PegLab’s three prices disagree after a 0.2 tKAS trade.

---

## 4. Best of this desk (why these, not the other 30)

Picked because a stranger can *do something* or *learn a law*. Everything else is a pass log, a fork, or a duplicate.
...
8. In-page inject: **Kasware or Kastle** only. Never ask for a seed.
...
| groks-wallet | **1,163,146.25** tKAS | TN10 address below |
...
TN10 wallet (worthless coins, used to break things):
...
Year 1 on this pin: one WorkCredit UTXO, receipts with live TN10 txids, one 402 endpoint that actually charges, Ishum hosted on a second machine, USDT labelled at the till. Covenant counts may stay small. Locked KAS in *our* UTXOs should be inventory, not TVL cosplay.
